...
> **狀態:已實作並進版。** `getShortcutDelta` 主鍵改為 `Cmd/Ctrl+Option+=/-/0`(改用 `event.code`);`manifest.json` commands 加上 `Cmd/Ctrl+Shift+↑/↓/0` 預設鍵。原 `Cmd+=`(無 Option)分支移除,讓原生縮放通過。in-page `Option+Shift+方向鍵` 舊備援已移除(改由 commands 提供方向鍵)。
...
> **狀態(2026-06-16):M1–M3 已全部實作並進版。** commits:M1 `6b8b009`、F1/F2 `1fb6ed4`、M2 `c88971b`、M3a(設定+badge)`8158a26`、M3b(Chrome 同步)`6b3e16c`;文件 `714f81d`、收尾 `6899e2a`。48 個測試通過、lint 乾淨。
>
> 仍延後(非本期範圍):無修飾鍵 `+/-` 選配(需設定開關)、file:// 專案搬移後的自動重新辨識(P0-4 穩定 ID / 路徑指紋)。
